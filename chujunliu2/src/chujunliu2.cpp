...
void laserCallback(const sensor_msgs::LaserScan& laser_scan) 
{
        ...
        for(float i=-90.0;i<90;i=i+5.0)  // for loop detect the distance from-90degree to 90 
   ...
   if(i>-37&&i<37)                   //  between -37 and 37  use different method 
   { 
        if (ping_dist_in_front_<0.4/sin(j+1.57))   
        {
   ...
   else if (ping_dist_in_front_<0.3/cos(1.57-fabs (j))) 
   {   float k=0.3/cos(1.57-abs (j));
    ROS_INFO("k = %f",k);
    ROS_INFO("j = %f",j);
    ROS_INFO("absj = %f",fabs(j));
       ROS_WARN("DANGER, WILL ROBINSON!!");
       laser_alarm_=true;break;         
   }                                           
   else laser_alarm_=false;
