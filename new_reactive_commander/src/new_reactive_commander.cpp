...
#include <new_reactive_commander/double_bool.h> // boolean message

bool g_lidar_alarm=false; // global var for lidar alarm
bool turndir;
void alarmCallback(const new_reactive_commander::double_bool& alarm_msg) 
{ 
  g_lidar_alarm = alarm_msg.bool1; //make the alarm status global, so main() can use it
  turndir=alarm_msg.bool2;
  if (g_lidar_alarm) {
...
int main(int argc, char **argv) {
    ...
    double yaw_rate = 1.0; //0.5 rad/sec yaw rate command
    ...
    while(ros::ok()) { // do forever
        ...
        else {twist_cmd.linear.x=0.0; //stop moving forward
        twist_cmd.angular.z=yaw_rate; //and start spinning in place
        timer=0.0; //reset the timer
        }
        while(g_lidar_alarm) {
